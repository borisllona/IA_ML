...
def buildtree_iter(part,scoref=entropy,beta=0):
	...
	fringe = []
	finalTree = decisionnode()
	fringe.append([part, finalTree])
	finalTree.results = None

	while(len(fringe)!=0):
		current_info = fringe.pop(0)
		
		current_part = current_info[0]
		current_node = current_info[1]

		t = scoref(current_part)
		best_gain = 0 #Impurity decrement, it has to be max.
		best_criteria = (None,None) #(col,value)
		best_sets = (None,None) #(set1,set2)

		for column in current_part:
			for index in range(0,len(column)-1):
				sets = divideset(current_part,index,column[index])
				tl = scoref(sets[0])
				tr = scoref(sets[1])
				current_score = t - (len(sets[0])/len(current_part)*tl) - (len(sets[1])/len(current_part)*tr)
		...
		if best_gain > beta:
			...
			current_node.tb = leftnode
			current_node.fb = rightnode
			current_node.col = best_criteria[0]
			current_node.value = best_criteria[1]

			fringe.append([set1, leftnode])
			fringe.append([set2, rightnode])
...
def test(sets,incrementTrainingSet=0):
	...
	printtree(buildtree(trainingS), ' ')
	print("################################")
	printtree(buildtree_iter(trainingS) , ' ')
